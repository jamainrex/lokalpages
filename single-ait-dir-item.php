...
if($thumbnailDir !== false){
	...
	$latteParams['thumbnailDir'] = getRealThumbnailUrl(getCategoryMeta("icon",intval(lp_get_the_term_id($item->ID))));
}
...
// Check if fullwidth layout
$full_enabled = get_post_meta( $latteParams['post']->id, '_serv_fullwidth_enable', true ); 
if( isset( $full_enabled ) && $full_enabled == 'y' )
    $latteParams['fullwidth'] = true;

$latteParams['rating'] = get_post_meta( $latteParams['post']->id, 'rating', true );

$latteParams['isServRevRateDisable'] = get_post_meta( $latteParams['post']->id, '_serv_revrate_disable', true );
$latteParams['isServGPSMapDisable'] = get_post_meta( $latteParams['post']->id, '_serv_gpsmap_disable', true );
...
$latteParams['options']['lp_after_listing_output'] = get_option( 'lp_after_listing_output' );
$latteParams['footerWidget'] = 'item';


$latteParams['facebookPageId'] = get_post_meta( $latteParams['post']->id, 'lp_listing_facebook_page_url', true );
