...
if( isset( $current_user->roles ) && is_array( $current_user->roles ) && ( in_array('directory_1', $current_user->roles) || in_array('directory_2', $current_user->roles) || in_array('directory_3', $current_user->roles) || in_array('directory_4', $current_user->roles) || in_array('directory_5', $current_user->roles) ) ) {
	...
	if (is_admin()) {
        ...
        function lp_remove_post_custom_fields() {
            //remove_meta_box( 'postcustom' , 'post' , 'normal' ); 
            remove_meta_box( 'tptn_metabox', 'ait-dir-item', 'advanced' );
            remove_meta_box( 'wpseo_meta', 'ait-dir-item', 'normal' );    
            remove_meta_box( 'crp_metabox', 'ait-dir-item', 'advanced' );    
                
        }
        add_action( 'do_meta_boxes' , 'lp_remove_post_custom_fields', 99 );
        
        function lpDirItemChangeColumns($cols) {
        ...
        add_filter( "manage_ait-dir-item_posts_columns", "lpDirItemChangeColumns", 99 );
