...
// rating
require_once dirname(__FILE__) . '/functions-rating.php';
// sorting
require_once dirname(__FILE__) . '/functions-sorting.php';
// easy admin
...
// Lokalpages - Custom functions
require_once dirname(__FILE__) . '/includes/lp_functions-dir.php';
...
add_filter('pre_get_posts','directoryMainPreGetPosts');
function directoryMainPreGetPosts($query) {
	...
	if ($query->is_main_query() && !$query->is_admin) {
		...
		if (!empty($_GET['pagination'])) {
			...
			if (isset($_GET['dir-search'])) {
				$num = (isset($aitThemeOptions->search->searchItemsPerPage)) ? $aitThemeOptions->search->searchItemsPerPage : 10;
	...
	foreach ($items as $key => $item) {
		...
		if(!empty($radius) && !isPointInRadius($radius[0], $radius[1], $radius[2], $item->optionsDir['gpsLatitude'], $item->optionsDir['gpsLongitude'])){
			...
			if($image !== false){
                ...
                //$item->thumbnailDir = getRealThumbnailUrl($GLOBALS['category']->icon);
...
function aitChangeAuthorForItems($output) {
	...
	if(isset($post) && $post->post_type == 'ait-dir-item') {
		$users = array();
...
function aitRegisterDirectoryUser( $user_login, $user_email ) {
    ...
    lp_new_user_notification( $user_id, $user_pass );
...
function aitContactOwner() {
    ...
    if ((!empty($_POST['name'])) && (!empty($_POST['from'])) && (!empty($_POST['to'])) && (!empty($_POST['subject'])) && (!empty($_POST['message']))) {
        ...
        $result = wp_mail( strip_tags($_POST['to']), "Contact Business", lp_aitContactOwner_bodyTemplate( strip_tags($_POST['subject']), strip_tags($_POST['message']), strip_tags($_POST['bizname']) ), $headers );
        if (!$result) {
...
add_action('wp_ajax_nopriv_ait_ask_business_services', 'aitAskBusinessServices');
add_action('wp_ajax_ait_ask_business_services', 'aitAskBusinessServices');
function aitAskBusinessServices() {
	...
	if ((!empty($_POST['name'])) && (!empty($_POST['from'])) && (!empty($_POST['to'])) && (!empty($_POST['subject'])) && (!empty($_POST['message']))) {
		...
		$headers[] = 'Content-Type: text/html; charset=UTF-8';
        //$result = wp_mail( strip_tags($_POST['to']), strip_tags($_POST['subject']), strip_tags($_POST['message']), $headers );
		$result = wp_mail( strip_tags($_POST['to']), "Ask Business", lp_aitContactOwner_bodyTemplate( strip_tags($_POST['subject']), strip_tags($_POST['message']), strip_tags($_POST['bizname']), strip_tags($_POST['contact']) ), $headers );
		if (!$result) {
			_e( 'Error with sending email', 'ait' );
			exit();
		}
...
function lp_aitContactOwner_bodyTemplate( $subject, $msg, $bizname, $contact ='' )
{

$contact_html = (!empty( $contact ) ?  '<h3>Contact: </h3>
    <p>'.$contact.'</p>' : '');

    $html = '<h3 style="font-weight: normal;">
    Subject: <strong>'.$subject.'</strong></h3>
    <br />
    Bisuness: <strong>'.$bizname.'</strong></h3>
<hr />
...
<div>
   <a href="http://lokalpages.com/" class="trademark">
        <img height="50" alt="logo" src="http://lokalpages.com/wp-content/uploads/2015/02/lp_logo_new.png" style="border: none;">
   </a>
</div>
<p style="font-size:10px;">This e-mail was sent through your listing at <a href="http://lokalpages.com/" class="trademark">lokalpages.com</a></p>';
...
if ( isset($aitThemeOptions->directory->enableClaimListing) ) {
	require_once dirname(__FILE__) . '/functions-claim.php';
}
