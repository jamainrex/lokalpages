...
if(isset($aitThemeOptions->directory->showTopCategories)){
	...
	$subcategories = get_terms( 'ait-dir-item-category', array('number' => 10 ,'hide_empty' => true, 'parent' => 0, 'orderby'=>'count', 'order'=>'DESC') );
	// add category links
...
if(isset($aitThemeOptions->directory->showTopLocations)){
	$sublocations = get_terms( 'ait-dir-item-location', array('number' => 10 ,'hide_empty' => true, 'parent' => 0, 'orderby'=>'count', 'order'=>'DESC') );
	// add category links
...
$latteParams['bbHeaderText'] = get_option( 'lp_landing_header_text_output2' );
//$latteParams['is_landing'] = is_front_page();
